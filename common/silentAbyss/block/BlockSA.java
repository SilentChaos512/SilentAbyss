...
public class BlockSA extends Block {
    ...
    /**
     * If set to true, the block will automatically get unlocalized names and
     * textures for each of the basic gem types.
     */
    ...
    public void registerIcons(IconRegister reg) {
        ...
        if (basicGemSubtypes) {
            ...
            blockIcon = reg.registerIcon(Strings.RESOURCE_PREFIX + getUnlocalizedName());
        }
    }

    /**
     * Fills the icon array (and creates it, if necessary) with icons
     * corresponding to the six basic gems.
     * 
     * @param reg
     * @param blockName
     */
    ...
    public void registerIconsForBasicGems(IconRegister reg) {
        ...
        for (int i = 0; i < EnumGem.basic().length; ++i) {
            icons[i] = reg.registerIcon(Strings.RESOURCE_PREFIX + this.blockName + EnumGem.basic()[i].name);
        }
    }

    /**
     * If the block hasSubtypes, tries to get an icon from the icons array (does
     * not error check the array). Otherwise, the blockIcon is returned.
     */
    @SideOnly(Side.CLIENT)
    @Override
    ...
    /**
     * If the block hasSubtypes, this will add one ItemStack to the subItems
     * list for each icon.
     */
    ...
    /**
     * Sets Block.unlocalizedName and blockName (because unlocalizedName is
     * private).
     */
    @Override
    ...
    public Block setHasBasicGemSubtypes(boolean value) {
    ...
    /**
     * This should be overridden in any deriving class and called in
