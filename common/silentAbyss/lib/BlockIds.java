package silentAbyss.lib;

public class BlockIds {

	// Default block ids
	public static int ABYSS_GEM_ORE_DEFAULT = 3200;
	public static int ABYSS_GEM_BLOCK_DEFAULT = 3202;
	public static int ABYSS_TELEPORTER_DEFAULT = 3204;
	public static int SIGIL_INFUSER_DEFAULT = 3205;
	public static int BRICK_DEFAULT = 3210;
	public static int BRICK_SLAB_DEFAULT = 3211;
	public static int BRICK_STAIRS_DEFAULT = 3212;
	public static int TEST_BLOCK_DEFAULT = 3300;
	
	// Current block ids
	public static int ABYSS_GEM_ORE;
	public static int ABYSSITE_ORE;
	public static int ABYSS_GEM_BLOCK;
	public static int ABYSSITE_BLOCK;
	public static int ABYSS_TELEPORTER;
	public static int SIGIL_INFUSER;
	public static int BRICK;
	public static int BRICK_SLAB;
	public static int BRICK_STAIRS;
	public static int TEST_BLOCK;
	
}
